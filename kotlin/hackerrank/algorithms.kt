...
fun stdin() : List<String> {
    if (System.`in`.available() == 0) { System.err.println("nothing in stdin") ; System.exit(1) }
    return generateSequence { readLine() }.toList()
}
public inline fun <T> Iterable<T>.sumLong(selector: (T) -> Long): Long {
    var sum: Long = 0L
    for (element in this) {
...
fun main(args: Array<String>) {
    val (n, m) = readLine()!!.toInts()
    val input = generateSequence { readLine() }
    println(addAndFindMax(n, m, input).max()!!)
}

// https://www.hackerrank.com/challenges/crush/problem fails with timeout
fun addAndFindMax(n : Int, m: Int, lines: Sequence<String>) : List<Long>  {
...
class AlgorithmTests {
    ...
    @Test fun max() {
        ...
        addAndFindMax(5, 3, input.asSequence()).toList() shouldBe expected
    }
